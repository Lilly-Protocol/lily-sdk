...
describe("validateHealthStatus (issue #437)", () => {
  it("passes through a valid HealthStatus payload unchanged", () => {
    const data = { status: "ok", version: "1.0.0", timestamp: "2024-01-01T00:00:00Z", checks: { db: "ok" } };
  ...
  it("throws VALIDATION_ERROR when version is not a string", () => {
    expect(() => validateHealthStatus({ status: "ok", version: 123, timestamp: "2024-01-01", checks: {} })).toThrow(
      "version",
    );
  });

  it("throws VALIDATION_ERROR when timestamp is not a string", () => {
    expect(() => validateHealthStatus({ status: "ok", version: "1.0.0", timestamp: 123, checks: {} })).toThrow(
      "timestamp",
    );
  });
  ...
  it("accepts status ok", () => {
    expect(validateHealthStatus({ status: "ok", version: "1.0.0", timestamp: "2024-01-01T00:00:00Z", checks: {} })).toEqual(
      { status: "ok", version: "1.0.0", timestamp: "2024-01-01T00:00:00Z", checks: {} },
    );
  });

  it("accepts status degraded", () => {
    expect(validateHealthStatus({ status: "degraded", version: "1.0.0", timestamp: "2024-01-01T00:00:00Z", checks: {} })).toEqual(
      { status: "degraded", version: "1.0.0", timestamp: "2024-01-01T00:00:00Z", checks: {} },
    );
  });

  it("accepts status down", () => {
    expect(validateHealthStatus({ status: "down", version: "1.0.0", timestamp: "2024-01-01T00:00:00Z", checks: {} })).toEqual(
      { status: "down", version: "1.0.0", timestamp: "2024-01-01T00:00:00Z", checks: {} },
    );
  });
});
...
describe("SystemClient with validateResponses=true (issue #437)", () => {
  it("validates health response when validateResponses is enabled", async () => {
    ...
    const mockRequest = vi
      .fn()
      .mockResolvedValue({ status: "ok", version: "2.0", timestamp: "2024-01-01T00:00:00Z", checks: { db: "ok" } });
  ...
  it("no longer uses unsafe as unknown cast", async () => {
    ...
    const mockRequest = vi
      .fn()
      .mockResolvedValue({ status: "ok", version: "1.0", timestamp: "2024-01-01T00:00:00Z", checks: {} });
    client.request = mockRequest as any;

    const result = await client.health();
    expect(result).toEqual({ status: "ok", version: "1.0", timestamp: "2024-01-01T00:00:00Z", checks: {} });
  });
});
