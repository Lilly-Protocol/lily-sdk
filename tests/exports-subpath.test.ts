...
describe('package exports subpaths', () => {
  const exportsMap = packageJson.exports as unknown as Record<
    string,
  ...
  for (const subpath of subpaths) {
    it(`${subpath} resolves to existing files with type declarations`, () => {
      ...
      if (typeof entry === 'string') {
        ...
        expect(
          existsSync(filePath),
          `Missing file for ${subpath}: ${entry}`,
        ).toBe(true);
        return;
      }
      ...
      if ('types' in entry && entry.types) {
        ...
        expect(
          ...
          `Missing types for ${subpath}: ${entry.types}`,
        ).toBe(true);
      }
      ...
      if ('import' in entry && entry.import) {
        ...
        expect(
          ...
          `Missing import for ${subpath}: ${entry.import}`,
        ).toBe(true);
      }
      ...
      if ('require' in entry && entry.require) {
        ...
        expect(
          ...
          `Missing require for ${subpath}: ${entry.require}`,
        ).toBe(true);
      }
    });
  }
