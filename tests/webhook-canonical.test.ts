...
describe("verifyWebhookJSON canonical serialization (issue #436)", () => {
  it("accepts signature from differently-key-ordered JSON", () => {
    const dataA = { a: 1, b: 2 };
    const dataB = { b: 2, a: 1 };
    const sigA = createHmac("sha256", SECRET).update(JSON.stringify(dataA)).digest("hex");
  ...
  it("still rejects tampered payloads", () => {
    const sig = createHmac("sha256", SECRET).update(JSON.stringify({ event: "test" })).digest("hex");
    expect(verifyWebhookJSON({ event: "tampered" }, sig, SECRET)).toBe(false);
  });

  it("handles arrays deterministically", () => {
    const sig = createHmac("sha256", SECRET).update(JSON.stringify({ items: [3, 1, 2] })).digest("hex");
